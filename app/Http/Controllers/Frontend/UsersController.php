...
class UsersController extends Controller
{
    ...
    {
        $categories = Category::whereStatus(1)->pluck('name', 'id'); // we use pluck to get only name and id from categories
    ...
    public function destroy_post_media($media_id)
    {
        ...
        if ($media) {
            if (File::exists('assets/posts/' . $media->file_name)) {
                unlink('assets/posts/' . $media->file_name);// it means delete image frim folder
            }
            $media->delete();
            return true;
        }
